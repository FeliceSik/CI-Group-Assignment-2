...
# ===========================
# DROP-IN SEARCH SETTINGS (easy delete later)
# ===========================
RUN_RATIO_SEARCH = True
RATIOS_TO_TRY = [0.3, 0.5, 0.7, 0.8, 0.9, 1.0]
# ===========================
...
# ============================================================
# 10) DROP-IN BLOCK: Search best TARGET_RATIO by Validation F1
#     Delete this whole block after you choose the best ratio.
# ============================================================
if RUN_RATIO_SEARCH:
    print("\n\n============================================================")
    print("DROP-IN: Searching for BEST TARGET_RATIO (by Validation F1)")
    print("You can delete this whole block after picking the best ratio.")
    print("============================================================")

    results = []  # IMPORTANT: reset so no old tuples remain

    for R in RATIOS_TO_TRY:
        # Oversample TRAIN only from the ORIGINAL (pre-oversampling) train split
        tmp_train_df = X_train_base.copy()
        tmp_train_df["Revenue"] = y_train_base.values

        maj = tmp_train_df[tmp_train_df["Revenue"] == 0]
        minn = tmp_train_df[tmp_train_df["Revenue"] == 1]

        target_n1 = int(R * len(maj))
        minn_over = resample(minn, replace=True, n_samples=target_n1, random_state=42)

        train_bal = pd.concat([maj, minn_over]).sample(frac=1, random_state=42)
        X_tr = train_bal.drop("Revenue", axis=1)
        y_tr = train_bal["Revenue"].astype(int)

        # Fit preprocessing + PCA on THIS train (no leakage)
        ...
        X_tr_proc = pre_tmp.fit_transform(X_tr)
        X_va_proc = pre_tmp.transform(X_val)

        X_tr_dense = to_dense(X_tr_proc)
        X_va_dense = to_dense(X_va_proc)

        pca_tmp = PCA(n_components=0.95, random_state=42)
        X_tr_pca_tmp = pca_tmp.fit_transform(X_tr_dense)
        X_va_pca_tmp = pca_tmp.transform(X_va_dense)

        # Train model for this ratio
        model_tmp = build_model(X_tr_pca_tmp.shape[1])
        ...
        # NOTE: keeping class_weight similar to your main code; you can remove if you want
        classes_tmp = np.unique(y_tr)
        cw_tmp = compute_class_weight(class_weight="balanced", classes=classes_tmp, y=y_tr)
        cw_dict_tmp = {cls: w for cls, w in zip(classes_tmp, cw_tmp)}

        model_tmp.fit(
            X_tr_pca_tmp, y_tr,
            validation_data=(X_va_pca_tmp, y_val),
            epochs=50,
            batch_size=64,
            class_weight=cw_dict_tmp,
            callbacks=[early_tmp],
            verbose=0
        )

        # Best threshold by VAL F1
        va_prob = model_tmp.predict(X_va_pca_tmp).ravel()
        thr = np.linspace(0, 1, 1001)
        f1s = [f1_score(y_val, (va_prob >= t).astype(int), zero_division=0) for t in thr]

        best_f1 = float(np.max(f1s))
        best_t = float(thr[int(np.argmax(f1s))])

        results.append((R, best_f1, best_t))
        print(f"Ratio={R:.1f}  ValF1={best_f1:.4f}  BestThr={best_t:.3f}")

    best_item = max(results, key=lambda x: x[1])
    best_ratio, best_f1, best_thr = best_item[0], best_item[1], best_item[2]

    print("\nBEST CHOICE (by validation F1):")
    print(f"TARGET_RATIO = {best_ratio} | Val F1 = {best_f1:.4f} | Best threshold = {best_thr:.3f}")
    print("============================================================")
